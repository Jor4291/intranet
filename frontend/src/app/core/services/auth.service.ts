...
export class AuthService {
  ...
  login(organizationSlug: string, username: string, password: string): Observable<LoginResponse> {
    return this.http.post<LoginResponse>(`${getCurrentApiUrl()}/login`, {
  ...
  register(data: any): Observable<LoginResponse> {
    return this.http.post<LoginResponse>(`${getCurrentApiUrl()}/register`, data).pipe(
      tap(response => {
        localStorage.setItem('token', response.token);
        localStorage.setItem('user', JSON.stringify(response.user));
        this.currentUserSubject.next(response.user);
  ...
  logout(): void {
    ...
    this.http.post(`${getCurrentApiUrl()}/logout`, {}).subscribe();
  }
  ...
  updateProfile(profileData: { first_name: string; last_name: string; email: string }): Observable<User> {
    return this.http.put<User>(`${getCurrentApiUrl()}/profile`, profileData).pipe(
      tap(updatedUser => {
  ...
  changePassword(passwordData: { current_password: string; password: string; password_confirmation: string }): Observable<any> {
    return this.http.post(`${getCurrentApiUrl()}/profile/change-password`, passwordData);
  }
}
